...
| **Strategy #4 — Graduation Snipe** | **Pump.fun → Raydium graduation entry agent** — `build_graduation(...)` with `rule` / `llm` / `hybrid` / `hybrid_audit` modes, `graduation_gate` (6 bonding-curve / pair checks), reflective loop on the LLM paths. New schemas: `GraduationEvent`, `GraduationConfig`, `GraduationContext`, `GraduationVerdict`. | ✅ done | v0.12.0 |
| **PL1 — Position Lifecycle Helpers** (position management, NOT a strategy) | **First position-management agent.** `build_lifecycle()` consumes `PositionContext`, emits `Decision(action ∈ {hold, take_profit, scale_out, exit_full}, size)`. Hard exits (emergency / SL / time stop) ALWAYS rule-deterministic, even in llm/hybrid mode. 4 modes: rule / llm / hybrid / hybrid_audit. Strictly recommendation — bot owns execution, slippage, MEV, persistence. NOT in [STRATEGIES.md](STRATEGIES.md) (universal risk-management primitive). | ✅ done | v0.13.0 |
| **S5 — Smart Money Confluence** (S-tier entry) | **Multi-wallet correlation entry agent** — fires when ≥ N pre-vetted smart wallets buy same token in window T. Distinct from KOL (multi-wallet correlation vs single-wallet copy). Validated as mainstream pro strategy ("5+ smart wallets scooping a token over a week"). See [STRATEGIES.md](STRATEGIES.md#s-tier--proven-by-mainstream-solana-pro-traders). | 📅 planned | — |
| **S6 — Early-Stage Dip Buy** (S-tier entry) | **One agent for two related events:** wait for the dump wave to settle, then enter on retention. Config `event_type ∈ {launch, graduation}` — **launch** = wait 1–10 min for sniper/bundler dump (still in BC); **graduation** = wait for early-BC-buyer TP wave after Raydium migration. Distinct timing window from speed sniper / graduation snipe. See [STRATEGIES.md](STRATEGIES.md#s-tier--proven-by-mainstream-solana-pro-traders). | 📅 planned | — |
| **A1 — Organic Growth Detector** (A-tier triage) | **Time-series chart-pattern recognizer** — promotes scanner candidates to higher-confidence on organic growth shape (steady climb + healthy pullbacks + unique-buyer count rising) vs manipulation tells (vertical, zero sells). Build after S-tier ships. See [STRATEGIES.md](STRATEGIES.md#a-tier--validated-narrower-constraints). | 📅 planned | — |
