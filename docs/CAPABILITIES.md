...
| **S5 — Smart Money Confluence** (S-tier entry) | **Multi-wallet correlation entry agent** — fires when ≥ N pre-vetted smart wallets buy same token in window T. `build_confluence(registry, ...)` with 4 modes; `make_confluence_gate(registry)` factory (wallet dedup, tier/hit_rate/sol checks, count gate); `SmartWalletRegistry` mirrors `KOLRegistry`. New schemas: `SmartWalletProfile`, `SmartWalletAccumulation`, `ConfluenceEvent`, `ConfluenceConfig`, `ConfluenceContext`, `ConfluenceVerdict`. See [STRATEGIES.md](STRATEGIES.md#s-tier--proven-by-mainstream-solana-pro-traders). | ✅ done | v0.14.0 |
| **S6 — Early-Stage Dip Buy** (S-tier entry) | **One agent for two related events.** `build_dip_buy()` with 4 modes; 4 gates in series: `timing_gate` (window), `dip_gate` (ATH drop + sell pressure), `recovery_gate` (buy-ratio, holder retention, buyers trend, price stability), `market_gate`. Sizing: recovery_score × dip_bonus × sell_calm. New schemas: `DipBuySnapshot`, `DipBuyConfig`, `DipBuyContext`, `DipBuyVerdict`. See [STRATEGIES.md](STRATEGIES.md#s-tier--proven-by-mainstream-solana-pro-traders). | ✅ done | v0.15.0 |
